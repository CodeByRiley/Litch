...
#pragma endregion
	...
	glm::ivec2 gl2d::Texture::GetSize()
	{
		glm::ivec2 s;
	...
	void gl2d::Texture::createFromBuffer(const char* image_data, const int width, const int height
	...
	void gl2d::Texture::create1PxSquare(const char* b)
	{
		if (b == nullptr)
		{
	...
	void gl2d::Texture::createFromFileData(const unsigned char* image_file_data, const size_t image_file_size
		,bool pixelated, bool useMipMaps)
	{
	...
	void gl2d::Texture::createFromFileDataWithPixelPadding(const unsigned char* image_file_data, const size_t image_file_size, int blockSize,
		bool pixelated, bool useMipMaps)
	{
		stbi_set_flip_vertically_on_load(true);
	...
	void gl2d::Texture::loadFromFile(const char* fileName, bool pixelated, bool useMipMaps)
	{
	...
	void gl2d::Texture::loadFromFileWithPixelPadding(const char* fileName, int blockSize,
		bool pixelated, bool useMipMaps)
	{
		std::ifstream file(fileName, std::ios::binary);
	...
	size_t gl2d::Texture::getMemorySize(int mipLevel, glm::ivec2 *outSize)
	{
		glBindTexture(GL_TEXTURE_2D, id);
	...
	void gl2d::Texture::readTextureData(void *buffer, int mipLevel)
	{
		glBindTexture(GL_TEXTURE_2D, id);
		glGetTexImage(GL_TEXTURE_2D, mipLevel, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
	}

	std::vector<unsigned char> gl2d::Texture::readTextureData(int mipLevel, glm::ivec2 *outSize)
	{
		glBindTexture(GL_TEXTURE_2D, id);
	...
	void gl2d::Texture::bind(const unsigned int sample)
	{
		glActiveTexture(GL_TEXTURE0 + sample);
		glBindTexture(GL_TEXTURE_2D, id);
	}

	void gl2d::Texture::unbind()
	{
		glBindTexture(GL_TEXTURE_2D, 0);
	}

	void gl2d::Texture::cleanup()
	{
